...
Current candidate: `0.4.2`, targeting DSH `0.1.0-rc.8` exactly. DSH rc.7 remains on the frozen Fleet `0.4.0` line.
...
- The `0.4.2` candidate has `21` Vitest files and `257` tests. Release gates are `pnpm run typecheck`, `pnpm test`, `pnpm run check:generated`, `pnpm run check:repository`, and `pnpm run release:check`.
