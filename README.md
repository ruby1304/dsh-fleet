...
`0.3.2` targets DSH `>=0.1.0-rc.7 <0.2.0`. It is an open-source preview with a deliberately narrow trust model: one owner, fixed devices, fixed SSH/local transports, and fixed workspace/profile policies.
...
Production profiles should install an exact npm release or a reviewed tarball. Never use a live checkout link as production state. Once `0.3.2` is published:

```bash
dsh plugin --profile web add dsh-fleet@0.3.2 --save-exact --ignore-scripts
```
...
shasum -a 256 dsh-fleet-0.3.2.tgz
dsh plugin --profile web add /absolute/path/to/dsh-fleet-0.3.2.tgz --save-exact --ignore-scripts
```
    ...
    convergence:
      ...
      targets:
        - deviceId: controller
          ...
          agentPath: /absolute/path/to/releases/0.3.2/agent.mjs
          configPath: /absolute/path/to/controller/agent.config.json
        - deviceId: worker
          transport: ssh
          sshHost: worker-mac
          nodeBinary: /opt/homebrew/bin/node
          agentPath: /Users/example/.local/share/dsh-fleet/releases/0.3.2/agent.mjs
